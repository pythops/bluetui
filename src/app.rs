...
impl App {
    pub async fn new(config: Arc<Config>, sender: UnboundedSender<Event>) -> AppResult<Self> {
        ...
        controller_state.select((!controllers.is_empty()).then_some(0));
    ...
    pub fn reset_devices_state(&mut self) {
        ...
        self.paired_devices_state
            .select((!controller.paired_devices.is_empty()).then_some(0));

        self.new_devices_state
            .select((!controller.new_devices.is_empty()).then_some(0));
    }
    ...
    pub fn render_set_alias(&mut self, frame: &mut Frame, area: Rect) {
        ...
        if let Some(selected_controller) = self.controller_state.selected() {
            let Some(index) = self.paired_devices_state.selected() else {
                return;
            };
            let controller = &self.controllers[selected_controller];
    ...
    pub fn render(&mut self, frame: &mut Frame) {
        let Some(selected_controller_index) = self.controller_state.selected() else {
            return;
        };
        ...
        //Adapters
        let controller_rows: Vec<Row> = self
            .controllers
            .iter()
            .map(|controller| {
                Row::new(vec![
        ...
        let num_controller_rows = controller_rows.len();

        let controller_table = Table::new(controller_rows, widths)
            .header({
        ...
        if num_controller_rows > controller_block.height.saturating_sub(4) as usize {
        ...
        //Paired devices
        let paired_device_rows: Vec<Row> = selected_controller
            .paired_devices
            .iter()
            .map(|d| {
                Row::new(vec![
            ...
            .collect();
        let num_paired_device_rows = paired_device_rows.len();

        if num_paired_device_rows > 0
            && self.focused_block == FocusedBlock::PairedDevices
        ...
        let paired_devices_table = Table::new(paired_device_rows, widths)
            .header({
                if show_battery_column {
        ...
        if num_paired_device_rows > paired_devices_block.height.saturating_sub(4) as usize {
            let scrollbar = Scrollbar::new(ScrollbarOrientation::VerticalRight)
                .begin_symbol(Some("↑"))
                .end_symbol(Some("↓"));
            let mut scrollbar_state = ScrollbarState::new(num_paired_device_rows)
        ...
        if render_new_devices {
            let new_device_rows: Vec<Row> = selected_controller
                .new_devices
                .iter()
                .map(|d| {
                    Row::new(vec![
                ...
                .collect();
            let num_new_device_rows = new_device_rows.len();

            let widths = [Constraint::Length(20), Constraint::Length(20)];

            let new_devices_table = Table::new(new_device_rows, widths)
                .header({
            ...
            if num_new_device_rows > new_devices_block.height.saturating_sub(4) as usize {
                let scrollbar = Scrollbar::new(ScrollbarOrientation::VerticalRight)
                    .begin_symbol(Some("↑"))
                    .end_symbol(Some("↓"));
                let mut scrollbar_state = ScrollbarState::new(num_new_device_rows)
    ...
    pub async fn refresh(&mut self) -> AppResult<()> {
        ...
        if adapter_removed {
            self.controller_state
                .select((!self.controllers.is_empty()).then(|| {
                    self.controller_state
                        .selected()
                        .unwrap_or(0)
                        .min(self.controllers.len() - 1)
                }));
        }
